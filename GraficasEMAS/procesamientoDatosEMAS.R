...
# Codigo valido solo para estaciones con 10 variables
...
# Leemos datos
datos <- read.delim("https://smn.cna.gob.mx/tools/GUI/EMAS_reporte_v2.php?id=616&formato=txt&tipo=90d") %>% 
...
datos3[,1] <- strptime(datos3[,1] %>% as.character(), 
              format = "%Y-%m-%d %H:%M")  - 21600#- (6/24) + (1&3600)
...
ggplot(data = datos3, aes(x = `AAAA/MM/DD HH:MM HORAZ`, y = `Radiación Solar (W/m²)`)) + 
  ...
  labs(title = paste0("Gráfica de radiación solar"), 
       subtitle = paste0("Estación: Cerro Catedral (Red: SMN EMAS)"),
       x = "Fecha (AAAA/MM/DD HH:MM)", 
       y = "Radiación Solar", 
       caption = "Elaboración propia con datos del SMN-Conagua. 2019.") +
  tema_juve + 
...
ggplot(data = datos3, aes(x = `AAAA/MM/DD HH:MM HORAZ`, y = datos3$`Humedad relativa (%)`)) + 
  ...
  labs(title = paste0("Gráfica de humedad relativa"), 
       subtitle = paste0("Estación EMAS tal"),
       x = "Fecha (AAAA/MM/DD HH:MM)", 
       y = "Humedad Relativa (%)", 
       caption = "Elaboración propia con datos del SMN-Conagua. 2019.") +
  tema_juve + 
