...
create_wordcloud_gobmex <- function(data, stop_words = c(), num_words = 100, background = "white",  mask = NULL) {
  ...
  wordcloud2(data, backgroundColor = background, figPath = mask,  color = c("#9d2449"), fontFamily = 'GMX Serif Boald') 
}

# Leemos el Plan Nacional de Desarrollo 
pnd <- pdftools::pdf_text("https://lopezobrador.org.mx/wp-content/uploads/2019/05/PLAN-NACIONAL-DE-DESARROLLO-2019-2024.pdf") %>%
  strsplit("\n") %>% 
  unlist()

# Hacemos el WordCloud
create_wordcloud_gobmex(data = pnd, num_words = 500, background = "black")
